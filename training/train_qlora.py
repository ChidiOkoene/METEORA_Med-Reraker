...
Loads a locally-downloaded Mixtral model and trains LoRA adapters using QLoRA-friendly setup.
- Local model path: C:\projects\meteora_project\Mixtral-8x7B-Instruct-v0.1
...
Run:
  accelerate launch --mixed_precision=fp16 training/train_qlora.py
"""
...
# ---------------- USER PATHS & HYPERPARAMS (already set to your provided paths) ----------------
LOCAL_MODEL_PATH = r"C:\projects\meteora_project\Mixtral-8x7B-Instruct-v0.1"
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(LOCAL_MODEL_PATH, use_fast=False, local_files_only=True)
    ...
    if not FORCE_SKIP_INFER:
        try:
            ...
            config = AutoConfig.from_pretrained(LOCAL_MODEL_PATH, trust_remote_code=True, local_files_only=True)
            ...
            logger.info(f"Inferred device_map summary: {{k: ('str' if isinstance(v,str) else 'list') for k,v in device_map.items()}}")
    ...
    if model is None:
        try:
            logger.info("Attempting simpler load with device_map='auto' (local files only)...")
            ...
            logger.info("Falling back to CPU-only load (local files). This will be slow.")
            model = AutoModelForCausalLM.from_pretrained(
                LOCAL_MODEL_PATH,
                quantization_config=bnb_config,
                device_map={"": "cpu"},
